...
def test_to_int_like_str_series():
    s = pd.Series(["1", "2.0", "003", "x", None])
    out = sub.to_int_like_str_series(s)
    # We only check the first four items; the None -> "None" behavior may vary across pandas versions
    assert out.tolist()[:4] == ["1", "2", "3", "x"]


def test_normalize_subject_series():
    s = pd.Series(["  Math ", "HeBrew", None])
    out = sub.normalize_subject_series(s)
    assert out.tolist() == ["math", "hebrew", "none"]


def test_active_and_container_masks():
    df = pd.DataFrame({
